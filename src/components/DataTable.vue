  ...
  <div class="data-table">
    <div class="filter-and-search-container">
        ...
        <div v-for="(filter, i) in filters" :key="i">
          ...
          <div v-if="filter.type === 'input-date'">
            <div class="input-with-icon date-input">
              ...
              <input type="date" v-model="filter.modal" :min="filter.min" :max="filter.max" />
            </div>
          </div>
          <!-- /input data -->
      ...
      <div class="input-with-icon search-input">
        ...
        <input v-if="search" placeholder="Pesquisar..." v-model="q" ref="search" />
      </div>
      <!-- /Pesquisa -->
    </div>

    <!-- Loop com v-for nos Headers e seus conteúdos -->
    <div class="data-container hidden-xs-only">
      <div v-for="(header, i) in headers" :key="i" :style="{ width: header.width }">
        <div class="header-container">
          <div class="header">{{ header.key }}</div>

          <!-- Sort para as propriedades -->
          <div class="sort-buttons" :style="{ visibility: header.sortable ? 'visible' : 'hidden' }">
            <i
        ...
        <!-- Loop com v-for para os itens da tabela -->
        <div class="content-container">
          <div class="content-row" v-for="(item, j) in items" :key="j">
            <!-- Conteúdo do Header com slot -->
            ...
            <!-- Ações -->
            <div v-if="header.type === 'actions'" class="actions hidden-xs-only">
              <div v-for="(action, k) in actions" :key="`action-${k}`" class="loop">
                <!-- Editar (ícone edit) -->
    ...
    <!-- /Loop dos Headers e Itens da Tabela para Desktop -->

    <!-- Loop dos Headers e Itens da Tabela para Mobile -->
    <div class="mobile-content hidden-sm-and-up">
      <div v-for="(items, i) in filteredItems" class="items-row" :key="i">
        <div v-for="(header, j) in headers" class="header-row" :key="`header-${j}`">
    ...
    <!-- Modal de ações -->
    <div class="edit-modal-card text-left">
      <v-menu offset-y v-model="editDialog">
        <v-list>
          <v-list-item v-for="(acao, index) in acoes" :key="index">
            <v-list-item-title>{{ acao.title }}</v-list-item-title>
          </v-list-item>
        </v-list>
      </v-menu>
    </div>
    ...
    <v-bottom-sheet v-model="mobileFilterDialog" width="300">
      <v-card class="bottom-sheet-card">
        ...
        <div class="close-n-clear-container">
          <button class="hidden-sm-and-up close-filter" @click="mobileFilterDialog = false">
            <i class="material-icons">keyboard_arrow_down</i>
          </button>

          <div class="filter-heading">Filtrar por:</div>
...
export default {
  ...
  data() {
    return {
      ...
      // /MOBILE FILTER DIALOG
      acoes: [{ title: "Editar" }, { title: "Arquivar" }, { title: "Excluir" }]
    };
  },
      ...
      // /CHECK IF NO FILTER SELECTED
        ...
        if (isAnyOneItemsArrEmpty) {
          ...
          if (arrOfItems.length === 1) {
            ...
            filteredItems = { arrOfArr: arrOfItems };
          }
          // /CHECK IF ONLY ONE FILTER SELECTED, NOTING TO FIND COMMON
        }
        // /CHECK IF ANYONE ITEMS ARR EMPTY, THIS MEANS NO COMMON
...
@import "./../assets/scss/data-table/data-table";
</style>